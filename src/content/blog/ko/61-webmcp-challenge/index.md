...
이미 배포해 둔 웹앱이 있다면 이번 기회에 직접 지원해봐도 좋겠다. 모든 기능을 한꺼번에 바꾸기보다 사용자가 자주 반복하는 흐름 하나를 골라, 목적이 분명한 WebMCP 도구 몇 개로 공개하는 편이 현실적이다. 사람이 같은 화면에서 에이전트의 결과를 확인하고 이어서 수정할 수 있는 장면까지 데모에 담으면 챌린지가 말하는 사람-에이전트 경험도 자연스럽게 보여줄 수 있다. 완전히 새로운 제품을 만들 필요가 없으므로 기존 앱이 있는 개발자에게는 짧게 실험해볼 만한 기회다.
